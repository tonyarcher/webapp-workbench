...
function roundStep(v: Uint32Array, wi: number, ki: number): void {
    const s1 = (rotr(v[4], 6) ^ rotr(v[4], 11) ^ rotr(v[4], 25)) >>> 0;
    const ch = ((v[4] & v[5]) ^ (~v[4] & v[6])) >>> 0;
    const t1 = (v[7] + s1 + ch + ki + wi) >>> 0;
    const s0 = (rotr(v[0], 2) ^ rotr(v[0], 13) ^ rotr(v[0], 22)) >>> 0;
    const maj = ((v[0] & v[1]) ^ (v[0] & v[2]) ^ (v[1] & v[2])) >>> 0;
    const t2 = (s0 + maj) >>> 0;
    v[7] = v[6];
    v[6] = v[5];
    v[5] = v[4];
...
function compressBlock(h: Uint32Array, w: Uint32Array): void {
    ...
    for (let i = 0; i < 64; i++) roundStep(v, w[i], K[i]);
