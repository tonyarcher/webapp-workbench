...
describe('BaseballLineupSetup', () => {
    ...
    it('renders player slots from lineup json attributes', async () => {
        ...
        element.setAttribute(
            'away-lineup-json',
            JSON.stringify([{ id: 1, name: 'Brendan Donovan', jerseyNumber: 3, position: 'LF' }]),
        );
        element.setAttribute(
            'home-lineup-json',
            JSON.stringify([{ id: 2, name: 'Nico Hoerner', jerseyNumber: 2, position: '2B' }]),
        );
        await element.updateComplete;
    ...
    it('emits close-lineup-setup on close and cancel button click', async () => {
        let closed = false;
        element.addEventListener('close-lineup-setup', () => {
            closed = true;
        });
    ...
    it('resyncs drafts when sync-token changes', async () => {
        ...
        element.setAttribute(
            'away-lineup-json',
            JSON.stringify([{ id: 1, name: 'Old Name', jerseyNumber: 1, position: 'CF' }]),
        );
        await element.updateComplete;
        element.setAttribute(
            'away-lineup-json',
            JSON.stringify([{ id: 1, name: 'New Name', jerseyNumber: 8, position: 'CF' }]),
        );
