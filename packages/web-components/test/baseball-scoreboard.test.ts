...
describe('BaseballScoreboard', () => {
    ...
    it('renders left-handed pitcher and batter with the result in the zone', async () => {
        element.setAttribute(
            ...
            JSON.stringify({
                gameState: {
                    currentBatterName: 'Ruth',
                    currentPitcherName: 'Grove',
                    batterBats: 'L',
                    pitcherThrows: 'L',
                },
            }),
        ...
        expect(
            element.shadowRoot!.querySelector('[data-testid="plate-pitcher"]')!.classList.contains('throws-L'),
        ).to.equal(true);
        expect(
            element.shadowRoot!.querySelector('[data-testid="plate-batter"]')!.classList.contains('bats-L'),
        ).to.equal(true);
    ...
    it('shows the glove and throwing arm for a right-handed pitcher', async () => {
        ...
        const pitcher = element.shadowRoot!.querySelector('[data-testid="plate-pitcher"]') as HTMLElement;
        expect(getComputedStyle(pitcher.querySelector('.glove-arm.side-right') as HTMLElement).display).to.equal(
            'block',
        );
        expect(getComputedStyle(pitcher.querySelector('.pitch-arm.side-left') as HTMLElement).display).to.equal(
            'block',
        );
        expect(getComputedStyle(pitcher.querySelector('.glove-arm.side-left') as HTMLElement).display).to.equal('none');
        expect(getComputedStyle(pitcher.querySelector('.pitch-arm.side-right') as HTMLElement).display).to.equal(
            'none',
        );
    });
    ...
    it('flies the ball only when a pitch location is set', async () => {
        ...
        element.setAttribute(
            'active-play-json',
            JSON.stringify({ eventType: 'STRIKE', strikeKind: 'looking', bats: 'R', throws: 'R' }),
        );
        await element.updateComplete;
