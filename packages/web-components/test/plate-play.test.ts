...
describe('plate-play', () => {
    ...
    it('uses looking to suppress the swing and falls back on handedness', () => {
        const play = parsePlatePlay(
            JSON.stringify({ eventType: 'STRIKE', strikeKind: 'looking', bats: 'X' }),
            'L',
            'L',
        );
    ...
    it('builds plate scene classes for result and swing', () => {
        expect(plateSceneClass('IN PLAY', true, true)).to.equal(
            'plate-scene result-IN-PLAY has-pitch swinging has-location animated',
        );
        expect(plateSceneClass('', false, false)).to.equal('plate-scene has-location instant');
        expect(plateSceneClass('STRIKE', false, false, false)).to.equal(
            'plate-scene result-STRIKE has-pitch no-location instant',
        );
        expect(plateSceneClass('BALL', false, false, true, true)).to.equal(
            'plate-scene result-BALL has-pitch has-location zone-interactive instant',
        );
    });
