...
const galleryItem: ScrollItem = {
    id: 6,
    title: 'G',
    body: '![a](https://x/a.png) ![b](https://x/b.png)',
...
assert(
    embedUrlFor('https://www.tiktok.com/@user/video/1234567890')?.includes('tiktok.com/player/v1/1234567890'),
    'tiktok @user/video embed',
);
...
assert(
    embedUrlFor('https://www.tiktok.com/embed/v2/1234567890')?.includes('player/v1/1234567890'),
    'tiktok embed/v2 url',
);
assert(
    embedUrlFor('https://www.tiktok.com/player/v1/1234567890')?.includes('player/v1/1234567890'),
    'tiktok player url',
);
assert(
    embedUrlFor('https://www.instagram.com/reel/CxYz123AbCd/')?.includes('/p/CxYz123AbCd/embed'),
    'instagram reel embed',
);
assert(embedUrlFor('https://www.instagram.com/p/CxYz123AbCd/')?.includes('/p/CxYz123AbCd/embed'), 'instagram p embed');
assert(
    embedUrlFor('https://www.instagram.com/someone/reel/CxYz123AbCd/')?.includes('/p/CxYz123AbCd/embed'),
    'instagram user/reel embed',
);
assert(
    embedUrlFor('https://www.instagr.am/reel/CxYz123AbCd/')?.includes('/p/CxYz123AbCd/embed'),
    'instagr.am reel embed',
);
...
assert(
    embedProviderForUrl('https://www.instagram.com/reel/CxYz123AbCd/')?.name === 'instagram',
    'instagram provider name',
);
...
);
{
    ...
    assert(
        provider?.isPlayerReadyMessage?.({ type: 'onPlayerReady', 'x-tiktok-player': true }),
        'tiktok player/v1 ready msg',
    );
    assert(
        !provider?.isPlayerReadyMessage?.({ type: 'onPlayerReady', 'x-tiktok-embed': true }),
        'embed/v2 flag ignored',
    );
    assert(
        !provider?.isPlayerReadyMessage?.({ type: 'onStateChange', 'x-tiktok-player': true }),
        'tiktok other msg ignored',
    );
    assert(
        provider?.parsePlayerMessage?.({ type: 'onStateChange', value: 1, 'x-tiktok-player': true })?.type ===
            'playing',
        'state playing',
    );
    assert(
        provider?.parsePlayerMessage?.({ type: 'onStateChange', value: 2, 'x-tiktok-player': true })?.type === 'paused',
        'state paused',
    );
    assert(
...
const FAKE: EmbedProvider = {
    name: 'fake',
    id(url) {
        return /example-embed\.test\/v\/(\w+)/.test(url) ? (url.match(/\/v\/(\w+)/)?.[1] ?? null) : null;
    },
    embedUrl(url) {
        const id = url?.match(/\/v\/(\w+)/)?.[1];
        return id ? `https://example-embed.test/embed/${id}` : null;
    },
    poster() {
        return null;
    },
...
assert(
    embedUrlFor('https://example-embed.test/v/abc')?.includes('example-embed.test/embed/abc'),
    'registered provider',
);

// stripImageProxy
assert(
    stripImageProxy('https://x/api/v3/image_proxy?url=https://real/img.png') === 'https://real/img.png',
    'proxy strip',
);
