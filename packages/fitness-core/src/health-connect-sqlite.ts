...
const INTERVAL: ColSpec[] = [
    ...
    {table: 'total_calories_burned_record_table', metric: 'energy', valueCol: 'energy', timeCol: 'start_time', toSi: (cal) => (cal / 1_000) * KCAL_TO_J},
    {table: 'active_calories_burned_record_table', metric: 'energy', valueCol: 'energy', timeCol: 'start_time', toSi: (cal) => (cal / 1_000) * KCAL_TO_J},
];
...
function mapHrSeries(rows: Record<string, unknown>[]): Sample[] {
    const buckets = new Map<number, {sum: number; n: number}>();
    for (const row of rows) {
        const t = asNumber(row.epoch_millis);
        const bpm = asNumber(row.beats_per_minute);
...
export function parseHealthConnectSqliteTables(tables: Record<string, Record<string, unknown>[]>): ParseResult {
    ...
    result.samples.push(...mapHrSeries(tables.heart_rate_record_series_table ?? []));
