...
export function personnelFromRosters(offenseRoster: Player[], defenseRoster: Player[]): Personnel {
    const offense = offenseRoster
        .filter((p) => p.unit === 'offense')
        .slice(0, 11)
        .map((p) => p.id);
    const defense = defenseRoster
