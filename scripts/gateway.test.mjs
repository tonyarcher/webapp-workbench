...
void describe("renderGatewayConf", () => {
  void it("renders plain HTTP byte-identically (no gaps)", () => {
    const out = renderGatewayConf(TEMPLATE, {});
    assert.equal(out, ["server {", "    listen 80;", "}"].join("\n"));
  });

  void it("adds the 443 listener and drops the 80 listener on redirect", () => {
...
void describe("sanEntry", () => {
  void it("maps IPs to IP entries and names to DNS", () => {
...
void describe("parseSanOutput", () => {
  void it("parses openssl subjectAltName output into comparable tokens", () => {
