...
class PasskeyCredentialRepository(private val store: PasskeyStore) : CredentialRepository {
    override fun getCredentialIdsForUsername(username: String): Set<PublicKeyCredentialDescriptor> =
        store.passkeysForUsername(username).map { row ->
    ...
    override fun getUsernameForUserHandle(userHandle: ByteArray): Optional<String> =
        Optional.ofNullable(store.usernameForHandle(userHandle.bytes))
    ...
    override fun lookupAll(credentialId: ByteArray): Set<RegisteredCredential> =
        store.lookupAll(credentialId.bytes).map {
            toRegistered(it)
        }.toSet()
}

private fun toRegistered(row: StoredPasskey): RegisteredCredential = RegisteredCredential.builder()
