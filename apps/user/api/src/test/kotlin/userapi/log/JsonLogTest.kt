...
class JsonLogTest {
    ...
    fun includesRequiredFields() {
        val line = formatLog("user-api", "info", "listening", mapOf("port" to 3000), now = now)!!
        ...
        assertEquals("user-api", obj["service"]?.jsonPrimitive?.content)
        assertEquals("3000", obj["port"]?.jsonPrimitive?.content)
    }

    @Test
    fun dropsDebugWhenMinIsInfo() {
        assertNull(formatLog("user-api", "debug", "request", minLevel = "info", now = now))
    }
