...
export function resolveInPlay(
    ...
    return (
        resolveHitQuality(quality, matchup, context, random) ??
        resolveInPlayOut(engine, matchup, context, random, quality)
    );
}
...
function resolveHitQuality(
    ...
    if (quality > 52) {
        return inPlayPlay('SINGLE', context, {
            location: pickItem(random, HIT_LOCATIONS),
            fieldPos: pickItem(random, [7, 8, 9, 4, 6]),
        });
    }
    return null;
}
...
function resolveInPlayOut(
    ...
    return (
        resolveRareReach(engine, context, random) ??
        resolveSacFly(engine, context, random, quality) ??
        resolvePlainOut(engine, matchup, context, random)
    );
}
...
function resolvePlainOut(
    ...
    if (chance(random, 0.22 + gb * 0.004)) {
        const doublePlay = Boolean(
            engine.runners[0] && engine.outs < 2 && chance(random, 0.28 - matchup.batter.ratings.speed * 0.002),
        );
    ...
    if (chance(random, 0.5))
        return inPlayPlay('LINE_OUT', context, pickItem(random, [...INFIELD_POS, ...OUTFIELD_POS]));
