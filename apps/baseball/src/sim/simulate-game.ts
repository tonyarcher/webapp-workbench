...
export function simulateGame(options: { home: SimRoster; away: SimRoster; seed: number; innings?: number }): {
    ...
    while (!engine.over && plays.length < MAX_PLAYS) {
        const play = nextPlay(
            engine,
            matchupFromGame(engine, options.home, options.away),
            rngForEngine(options.seed, engine, plays.length),
        );
