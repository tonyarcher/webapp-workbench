...
import { runnerAdvancementsForHit, runnerAdvancementsForWalk } from './notation';
import {
    addTeamScore,
    advanceRunnerState,
...
function recordHitStats(
    ...
    const withArcs = applyRunnerAdvancements(
        result,
        runnerAdvancementsForHit(pre.runners, bases),
        pre.runnerSlots,
        pre.runnerInnings,
    );
...
export function handleReachOnError(
    game: EngineGameState,
    eventType: ScoringEventType,
    fieldPos?: number,
): EngineGameState {
...
function handleSacBuntAdvance(withCell: EngineGameState, pre: EngineGameState): EngineGameState {
    ...
    const withArcs = applyRunnerAdvancements(
        withRbi,
        runnerAdvancementsForWalk(pre.runners),
        pre.runnerSlots,
        pre.runnerInnings,
    );
