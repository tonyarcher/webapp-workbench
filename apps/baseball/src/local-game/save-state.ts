...
function hasMetaShape(value: Record<string, unknown>): boolean {
    ...
    return (
        halfOk && runnersOk && idxOk && typeof value['totalInnings'] === 'number' && typeof value['over'] === 'boolean'
    );
}
...
export async function clearGameState(db: Promise<IDBPDatabase<BaseballDB>> = openGameDB()): Promise<void> {
    const database = await db;
