...
function applyModelEvent(
    model: ScorebookModel,
    event: ScoringEvent,
    rowCount: number,
): { runs: number; touched: string[] } {
    ...
    const forceDoublePlay =
        Boolean(event.doublePlay) && model.outs <= 1 && model.runners.some((runner) => runner.base === 1) && !sacFly;
    ...
    it('fills in correct base-path edges and advancement arcs across hundreds of games', () => {
        ...
        for (let gameSeed = 1; gameSeed <= GAMES; gameSeed++) {
            ...
            while (!game.over && eventsApplied < MAX_EVENTS) {
                ...
                const modelResult = applyModelEvent(
                    model,
                    event,
                    preHalf === 'TOP' ? pre.awayLineup.rows.length : pre.homeLineup.rows.length,
                );
