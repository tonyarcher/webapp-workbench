...
export function rostersOf(game: LiveLocalGameState): { home: SimRoster; away: SimRoster } {
    ...
    return {
        away: rosterFromLineup(
            random,
            game.setup.awayTeamName,
            game.setup.awayLineup ?? [],
            game.setup.awayPitcherName ?? '',
        ),
        home: rosterFromLineup(
            random,
            game.setup.homeTeamName,
            game.setup.homeLineup ?? [],
            game.setup.homePitcherName ?? '',
        ),
    };
}
