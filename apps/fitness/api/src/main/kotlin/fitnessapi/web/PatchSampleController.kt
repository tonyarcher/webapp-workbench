...
class PatchSampleController(private val samples: ObjectProvider<SampleStore>) {
    ...
    fun patch(@RequestBody body: JsonNode): Any {
        ...
        if (metric == null ||
            ...
            throw ApiException(HttpStatus.BAD_REQUEST, "metric and originId required")
        }
        val store = samples.orOffline()
        val row = store.findByOrigin(LOCAL_USER_ID, metric, originId)
            ?: throw ApiException(HttpStatus.NOT_FOUND, "sample not found")
        return applyPatch(store, row, body.jsonNumber("valueSi"), body.bool("hidden") == true, body.string("note"))
    }
}

private fun applyPatch(store: SampleStore, row: StoredSample, valueSi: Double?, hidden: Boolean, note: String?): Any {
    if (valueSi != null) {
        if (!valueSi.isFinite()) throw ApiException(HttpStatus.BAD_REQUEST, "valueSi must be finite")
    ...
    throw ApiException(HttpStatus.BAD_REQUEST, "valueSi or hidden required")
}
