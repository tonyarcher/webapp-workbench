...
class JdbcUrlsTest {
    ...
    fun parsesPostgresUrl() {
        val t = parseDatabaseUrl("postgres://rss:s3cret@postgres:5432/stock")
        assertEquals("jdbc:postgresql://postgres:5432/stock", t.jdbcUrl)
        assertEquals("rss", t.user)
        assertEquals("s3cret", t.password)
        assertEquals("stock", t.database)
    ...
    fun parsesPostgresqlUrl() {
        val t = parseDatabaseUrl("postgresql://u:p@localhost:5432/stock")
        assertEquals("jdbc:postgresql://localhost:5432/stock", t.jdbcUrl)
        assertEquals("stock", t.database)
