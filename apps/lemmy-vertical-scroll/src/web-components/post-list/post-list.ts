...
export class PostList extends LitElement {
    ...
    private renderList(): TemplateResult {
        return html`<div class="list-scroller" ${ref((el) => {
            ...
            (item) => {
                ...
                return html`<div class="list-item" data-index=${item.index} style="transform: translateY(${item.start}px)" ${ref((el) => this.virtualizer.measureElement(el as HTMLElement | null))}><lvs-post-card .post=${post}></lvs-post-card></div>`;
            },
        )}</div><div class="list-more">${this.renderMore()}</div></div>`;
    }
    ...
    private renderState(): TemplateResult {
        ...
        if (this.posts.length === 0)
            return html`<div class="list-state"><p class="state-title">Nothing here yet</p></div>`;
