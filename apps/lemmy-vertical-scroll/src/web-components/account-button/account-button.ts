...
export class AccountButton extends LitElement {
    ...
    private renderDropdown(): TemplateResult {
        ...
        return html`<form class="login-dropdown" @submit=${(e: Event) => {
            e.preventDefault();
            void this.onSubmit();
        }}><input class="account-username" type="text" placeholder="Username or email" autocomplete="username" .value=${this.username} @input=${this.onUsernameInput}><input class="account-password" type="password" placeholder="Password" autocomplete="current-password" .value=${this.password} @input=${this.onPasswordInput}>${this.renderTotp()}${this.error ? html`<p class="account-error">${this.error}</p>` : nothing}<button class="login-submit" type="submit" .disabled=${this.busy}>${this.busy ? 'Logging in…' : 'Log in'}</button></form>`;
    }
