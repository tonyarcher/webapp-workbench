...
export class AppShell extends LitElement {
    ...
    private renderViewModeSelect(value: ViewMode): TemplateResult {
        return html`<select class="sort-select" title="Feed view" @change=${this.onViewModeChange}>
            ${(['list', 'scroll'] as const).map(
                (mode) =>
                    html`<option value=${mode} ?selected=${mode === value}>${mode === 'list' ? 'List' : 'Scroll'}</option>`,
            )}
        </select>`;
    }
    ...
    private renderBootError(): TemplateResult {
        const detail =
            this.siteController.value.error instanceof Error
                ? this.siteController.value.error.message
                : String(this.siteController.value.error ?? '');
    ...
    private renderFeed(settings: Settings, postSort: PostSort): TemplateResult {
        if (settings.viewMode === 'scroll')
            return html`<lvs-scroll-feed .instance=${this.instance} .feedType=${this.clampFeedType(settings.feedType)} .sort=${postSort} .software=${this.software} .nsfwFilter=${settings.nsfwFilter} .auth=${this.authJwt}></lvs-scroll-feed>`;
