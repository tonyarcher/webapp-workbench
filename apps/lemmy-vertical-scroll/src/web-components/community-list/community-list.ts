...
export class CommunityList extends LitElement {
    ...
    private renderList(): TemplateResult {
        return html`<div class="list-scroller" ${ref((el) => {
            this.listEl = el as HTMLElement | null;
        })}><div class="list-spacer" style="height: ${this.virtualizer.totalSize}px">${this.virtualizer.virtualItems.map(
            (item) => {
                const community = this.communities[item.index];
                return html`<div class="list-item" data-index=${item.index} style="transform: translateY(${item.start}px)" ${ref((el) => this.virtualizer.measureElement(el as HTMLElement | null))}><lvs-community-card .community=${community}></lvs-community-card></div>`;
            },
        )}</div><div class="list-more">${this.query.isFetchingNextPage ? html`<span class="spinner" aria-label="Loading more"></span>` : html`<span class="more-hint"></span>`}</div></div>`;
    }
    ...
    private renderState(): TemplateResult {
        ...
        if (this.communities.length === 0)
            return html`<div class="list-state"><p class="state-title">No communities found</p></div>`;
