...
class AiQuotaService(
    ...
    fun consume(userId: UUID, hourlyLimit: Int, dailyLimit: Int) {
        try {
            ...
            throw ApiException(429, "AI budget used")
    ...
    private fun consumeOnce(userId: UUID, hourlyLimit: Int, dailyLimit: Int) {
        ...
        if (row.hourCount >= hourlyLimit) throw ApiException(429, "hourly AI budget used")
        if (row.dayCount >= dailyLimit) throw ApiException(429, "daily AI budget used")
        row.hourCount += 1
        row.dayCount += 1
