...
function buildArticle(feedId: string, item: ParsedItem, popularity: number, engagement: number): Article {
...
function collectLinks(items: ParsedItem[]): Set<string> {
    return new Set(
        items.map((it) => (it.link ? normalizeLink(it.link) : undefined)).filter((l): l is string => Boolean(l)),
    );
}

async function buildLinkInfo(
    links: Set<string>,
    feedId: string,
): Promise<Map<string, { matches: Article[]; otherFeeds: number; minPublished: number | undefined }>> {
    ...
    for (const link of links) {
        ...
        for (const mm of matches)
            if (mm.published < (minPublished ?? Number.POSITIVE_INFINITY)) minPublished = mm.published;
        m.set(link, { matches, otherFeeds: feeds.size, minPublished });
    }
    return m;
}

function buildMetaKeys(
    feed: Feed,
    hosts: Set<string>,
    authors: Set<string>,
    linkInfo: Map<string, { matches: Article[] }>,
): string[] {
...
function bumpsFor(
    itemInfo: { matches: Article[]; otherFeeds: number; minPublished: number | undefined },
    affMap: Map<string, number>,
    feedId: string,
): Map<string, BumpSpec> {
    const velocity = velocityBonus(
        itemInfo.otherFeeds,
        itemInfo.minPublished !== undefined ? Date.now() - itemInfo.minPublished : undefined,
    );
...
function itemMetrics(
    item: ParsedItem,
    linkInfo: Map<string, { matches: Article[]; otherFeeds: number; minPublished: number | undefined }>,
    now: number,
): {
    normLink: string | undefined;
    info: { matches: Article[]; otherFeeds: number; minPublished: number | undefined } | undefined;
    popularity: number;
    velocity: number;
} {
    const normLink = item.link ? normalizeLink(item.link) : undefined;
    ...
    const velocity = normLink
        ? velocityBonus(otherFeeds, info?.minPublished !== undefined ? now - info.minPublished : undefined)
        : 0;
    return { normLink, info, popularity, velocity };
}

function mergeBumps(
    bumpsByLink: Map<string, Map<string, BumpSpec>>,
    normLink: string,
    specs: Map<string, BumpSpec>,
): void {
    let existing = bumpsByLink.get(normLink);
    if (!existing) {
        existing = new Map<string, BumpSpec>();
        bumpsByLink.set(normLink, existing);
    }
    for (const [k, v] of specs) if (!existing.has(k)) existing.set(k, v);
}

function buildItemsAndBumps(
    parsed: ParsedFeed,
    feed: Feed,
    linkInfo: Map<string, { matches: Article[]; otherFeeds: number; minPublished: number | undefined }>,
    affMap: Map<string, number>,
    feedAffinity: number,
    now: number,
): { items: Article[]; bumpsByLink: Map<string, Map<string, BumpSpec>> } {
    const items: Article[] = [];
