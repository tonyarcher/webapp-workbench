...
class OpenApiConfigTest {
    ...
    @Test
    fun infoHasTitle() {
        val api = config.stockOpenApi()
        assertEquals("stock-game-api", api.info.title)
    ...
    fun customizerAddsVersionHeaderToDataRoutes() {
        ...
        open.paths(
            Paths().apply {
                addPathItem("/config", PathItem().get(Operation()))
        ...
        val configParams = open.paths["/config"]?.get?.parameters.orEmpty()
        assertTrue(configParams.any { it.name == "X-Api-Version" })
    ...
    fun customizerSkipsPresentParamAndEmptyPaths() {
        ...
        open.paths(
            Paths().apply {
                ...
                addPathItem("/orders", PathItem().get(withParam))
            },
        )
        config.versionHeaderCustomizer().customise(open)
        assertEquals(1, open.paths["/orders"]?.get?.parameters?.size)
