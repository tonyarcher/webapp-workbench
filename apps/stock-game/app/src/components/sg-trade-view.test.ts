...
describe('sg-trade-view', () => {
  ...
  it('searches symbols and jumps on select', async () => {
    ...
    try {
      ...
      tradeForm(el)?.dispatchEvent(
        new CustomEvent('sg-symbol-search-input', { detail: { query: 'aa' } }),
      )
      await settled()
      expect(calls.some((c) => c.includes('/search?q=aa'))).toBe(true)
      tradeForm(el)?.dispatchEvent(
        new CustomEvent('sg-symbol-select', { detail: RESULT }),
      )
      await settled()
      expect(window.location.hash).toBe('#/trade?symbol=AAPL')
      expect(
        tradeForm(el)?.shadowRoot?.textContent,
      ).toContain('$234.56')
      el.remove()
    } finally {
  ...
  it('drops the stale quote when the selection changes', async () => {    const msft = { ...RESULT, symbol: 'MSFT', name: 'Microsoft Corp.' }
