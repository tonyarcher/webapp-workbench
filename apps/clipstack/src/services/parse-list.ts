...
export function isTiktokHost(hostname: string): boolean {
    ...
    return (
        host === 'tiktok.com' || host.endsWith('.tiktok.com') || host === 'tiktokv.com' || host.endsWith('.tiktokv.com')
    );
}
...
function parseInstagram(
    ...
    skipped: SkippedLink[],
    seen: Set<string>,
): { id: string; author: string | null } | null {
...
function parseUrlEntry(
    ...
    skipped: SkippedLink[],
    seen: Set<string>,
): { provider: ClipProvider; id: string; author: string | null } | null {
